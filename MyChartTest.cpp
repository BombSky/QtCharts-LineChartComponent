...
int timeId;
MyChartTest::MyChartTest(QGraphicsItem *parent, Qt::WindowFlags wFlags)
{
    m_chart = new QChart;
    ...
    axisX->setGridLineVisible(false);//网格
...
//    m_chart->addAxis(axisY, Qt::AlignLeft);//居左
   ...
   //m_chart->setTitle("example of chart");   //设置图表标题
    ...
    m_chart->legend()->setVisible(false);  //设置图例可见
  //生成一小段数据列表用作绘图初始数据
...
//    }
    mydata1 << QPointF(1, 5) << QPointF(3, 7) << QPointF(7, 6) << QPointF(9, 7) << QPointF(12, 6)
    ...
    series->append(mydata1);  //将数据加到曲线中
...
MyChartTest::~MyChartTest()
{

}
void MyChartTest::addSeries(QList<QPointF> &data)  //用于新增曲线
{
...
void MyChartTest::removeSeries()  //移除一条曲线
{
...
//void MyChartTest::connectMarkers()  //将槽函数与图例的鼠标点击事件连接起来
//{
//    // Connect all markers to handler
//    foreach(QLegendMarker* marker, m_chart->legend()->markers()) {
//        // Disconnect possible existing connection to avoid multiple connections
//        QObject::disconnect(marker, &QLegendMarker::clicked, this, &MyChartTest::handleMarkerClicked);
//        QObject::connect(marker, &QLegendMarker::clicked, this, &MyChartTest::handleMarkerClicked);
//    }
//}

//void MyChartTest::disconnectMarkers()
//{
//    foreach(QLegendMarker* marker, m_chart->legend()->markers()) {
//        QObject::disconnect(marker, &QLegendMarker::clicked, this, &MyChartTest::handleMarkerClicked);
//    }
...
void MyChartTest::handleMarkerClicked()//图例点击事件
{
...
QList<QPointF> MyChartTest::setdata()  //设置图表数据的函数接口
{
    QList<QPointF> datalist;
    for (int i = 0; i < 100; i++)
        datalist.append(QPointF(i, i*0.1));
    return datalist;
}

void MyChartTest::timerEvent(QTimerEvent *event)    //定时器事件的重构
{
