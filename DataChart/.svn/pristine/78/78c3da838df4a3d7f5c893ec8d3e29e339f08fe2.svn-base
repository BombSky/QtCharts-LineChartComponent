...
public:
    QChart* pLineChart;     //图表组件，可理解为画笔，用它画曲线
    QChartView* pLineChartView;

    QValueAxis *YaxisStyle;             //y坐标轴
//    QValueAxis *XaxisStyle;          //时间类型轴(用作X轴)
    QDateTimeAxis *XaxisStyle;          //时间类型轴(用作X轴)
//    QDateTime *XaxisRange;
    int XaxisRange;
    QVBoxLayout *pVLayoutLineChart;
    RangeSlider *pRangeXAxisControlTwoSpinBox;
...
private:
    ...
    void XAxisAdaptive(const qint64 &newMaxTime);
    void SetXAxisMin();
public slots:
